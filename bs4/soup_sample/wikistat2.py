...
def build_tree(start, end, path):
    ...
    while links_to_parse:
        link_to_parse = links_to_parse.popleft()
        if link_to_parse == end:
            continue

        downlinks = get_downlinks(link_to_parse)

        if link_to_parse in downlinks:
            downlinks.remove(link_to_parse)

        for link in downlinks:
            if not files[link]:
                files[link] = [link_to_parse]
            else:
                files[link].append(link_to_parse)

            if link not in parsed_links and ''.join(['/wiki/', link]) in files_set:
